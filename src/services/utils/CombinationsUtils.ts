export function* getConstantLenghtCombinations<T extends string>(array: T[], combinationItemsCount: number, start: number = 0, prev: T[] = []): Iterable<T[]> {
    ...
    for(let i = start; i <= array.length - combinationItemsCount; i++) {
        ...
        // @ts-ignore
        yield* getConstantLenghtCombinations(array, combinationItemsCount - 1, i + 1, [...prev, array[i]]);
    }
}
